	...
	class FSubmitter extends FDataPump
	{
		...
		public function Show()
		{
			...
			switch ($this->Params->get("submittype"))
			{
				case 1:
					// Submit input
					$result .= '<input class="foxbutton" type="submit" style="margin-' . $GLOBALS["right"] . ':45%;" name="' . $this->GetId() . '" value="' . $this->Params->get("submittext") . '"/>' . PHP_EOL;
					break;
				...
				default:
					...
					$result .= '<button class="foxbutton" type="submit" style="width: 200px;border-radius: 10%; margin-' . $GLOBALS["left"] . ':35%;" name="' . $this->GetId() . '">' . PHP_EOL .
					'<span ';
